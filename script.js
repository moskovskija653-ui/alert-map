    ...
    try {
        ...
        if (geoJsonLayer) {
            geoJsonLayer.eachLayer(l => {
                ...
                // Собираем все возможные имена из свойств объекта
                const nameStr = JSON.stringify(p).toLowerCase();
                
                let foundStatus = "safe";
                for (let key in statusData) {
                    if (nameStr.includes(key.toLowerCase())) {
                        foundStatus = statusData[key];
                        break;
                    }
                }
                l.setStyle(getStyle(foundStatus));
            });
        }
...
async function init() {
    ...
    // Используем проверенный источник гео-данных
    ...
    geoJsonLayer = L.geoJson(geoData, {
        ...
        onEachFeature: (f, layer) => {
            layer.bindTooltip(f.properties.name_ru || f.properties.name, { sticky: true });
        }
    }).addTo(map);
    ...
    crimeaLayer.bindTooltip("Крым", { sticky: true });
